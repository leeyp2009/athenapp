...
if (nPlanet==2){
  ...
  Pp_time          = (pin->GetOrAddReal("problem",   "Pp_time",    0.0))*TWO_PI; // increase timescale for planets
...
/*
           int ic, jc, kc;
           Real x1_0 = PS[i].getRad();
           Real x2_0 = PS[i].getPhi();
           //x3_0 = PS[i].getPhi();
           for (ic=is; ic<=ie; ic++)
           ...
           //kc--;
           Real dx1 = pmb->pcoord->dx1f(ic);
           Real dx2 = pmb->pcoord->dx2f(jc);
           //cout << "x1: " << pmb->pcoord->x1f(ic) << ",x2: "<< pmb->pcoord->x2f(jc) << std::endl;
           //cout << "rp: " << x1_0 << ",phip: "<< x2_0 << std::endl;
           //cout << "dx1: " << dx1 << ",dx2: "<< dx2 << std::endl;
*/
/*
           Real dx1_, dx2_;
#ifdef MPI_PARALLEL
       MPI_Allreduce(&dx1,&dx1_,1,MY_MPI_REAL,MPI_MAX,MPI_COMM_WORLD);
       MPI_Allreduce(&dx2,&dx2_,1,MY_MPI_REAL,MPI_MAX,MPI_COMM_WORLD);
#endif
       NT = std::max(NT,PS[i].NperHydro(dt,dx1_,dx2_));
*/
...
//#pragma omp simd
	...
	for (int np=0;np<nPlanet;np++) {
          ...
          Real  Hill_radius = (std::pow(planet_gm/gm0*ONE_3RD, ONE_3RD)*rad_planet_new);
          Real rad_sft1 = rad_soft*Hill_radius; // 0.6 *r_Hill

          x_dis = rad_arr*std::cos(phi_arr) - rad_planet_new*std::cos(phi_planet_new);
          y_dis = rad_arr*std::sin(phi_arr) - rad_planet_new*std::sin(phi_planet_new);
          z_dis = z_arr - z_planet;

          rad_dis = rad_arr - rad_planet_new*cos(phi_arr - phi_planet_new);
          phi_dis = rad_planet_new*sin(phi_arr - phi_planet_new);
          ...
          if (PlanetaryGrvaityOrder == 2) {
            Real sec_g = planet_gm/pow(distance_square+SQR(rad_sft1), 1.5);
          ...
          if (PlanetaryGrvaityOrder == 4) {
            Real forth_g = planet_gm*(5.0*SQR(rad_sft1)+2.0*distance_square)/
          ...
          if (PlanetaryGrvaityOrder == 6) {
            Real sixth_g = planet_gm*(35.0*SQR(SQR(rad_sft1))+28.0*SQR(rad_sft1)*distance_square+
          ...
          if (IndirectTerm) {
             Real temp = planet_gm/SQR(rad_planet_new);
             acc_r   += -temp*std::cos(phi_arr - phi_planet_new);
	     acc_phi += temp*std::sin(phi_arr - phi_planet_new);
	     acc_z   += -temp*z_planet/rad_planet_new;
          }
          }
...
//ramp-up the disk effect using time pmass_incr_time
	  if (time < Pp_time) {
             Real rate = std::sin(time/(2.0*Pp_time)*PI);
             acc_r *= (rate*rate);
             acc_phi *= (rate*rate);
             acc_z *= (rate*rate);
          }
...
//#pragma omp simd
        ...
        acc_phi = 0.0;
        acc_theta = 0.0;
        GetCylCoord(pmb->pcoord, rad_arr, theta_arr, phi_arr, i, j, k);
	...
	for (int np=0;np<nPlanet;np++) {
          ...
          x_dis = rad_arr*std::cos(phi_arr)*std::sin(theta_arr) - 
		             rad_planet_new*std::cos(phi_planet_new)*std::sin(theta_arr);
          y_dis = rad_arr*std::sin(phi_arr)*std::sin(theta_arr) - 
		             rad_planet_new*std::sin(phi_planet_new)*std::sin(theta_arr);
          z_dis = rad_arr*std::cos(theta_arr)- rad_planet_new*std::cos(rad_planet_new);

          rad_dis = rad_arr - rad_planet_new*std::sin(theta_arr)*std::cos(phi_arr - phi_planet_new);
          theta_dis = -rad_planet_new*std::cos(theta_arr)*std::cos(phi_arr - phi_planet_new);
          phi_dis = rad_planet_new*std::sin(phi_arr - phi_planet_new);
          ...
          if (IndirectTerm) {
             Real temp = planet_gm/SQR(rad_planet_new);
             acc_r   += -temp*std::cos(phi_arr - phi_planet_new)*std::sin(theta_arr);
	     acc_theta += -temp*std::cos(phi_arr - phi_planet_new)*std::cos(theta_arr);
	     acc_phi   += +temp*std::sin(phi_arr - phi_planet_new);
          }
          }
...
void MassTransferWithinHill(MeshBlock *pmb, const Real time, const Real dt, const AthenaArray<Real> &prim,
  ...
  Real  Hill_radius = (std::pow(gmp/gm0*ONE_3RD, ONE_3RD)*rad_planet_new);
