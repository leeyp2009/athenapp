...
Real PlanetAccretionJdotHistory(MeshBlock *pmb, int iout);
Real PlanetAccretionEdotHistory(MeshBlock *pmb, int iout);
...
if (nPlanet==2){
 ...
 if(nPlanet>1) {
  ...
  if (nPlanet>0) {
    ...
    AllocateUserHistoryOutput(3*nPlanet);
    for(int n=0;n<nPlanet;n++) { 
     ...
     djdot_name << "djdot_" << n + 1;
     dedot_name << "dedot_" << n + 1;
     EnrollUserHistoryOutput(3*n, PlanetAccretionHistory, dmass_name.str().c_str(),UserHistoryOperation::sum); // default: sum
     EnrollUserHistoryOutput(3*n+1, PlanetAccretionJdotHistory, djdot_name.str().c_str(),UserHistoryOperation::sum); // default: sum
     EnrollUserHistoryOutput(3*n+2, PlanetAccretionEdotHistory, dedot_name.str().c_str(),UserHistoryOperation::sum); // default: sum
    }
 }
...
//    AthenaArray<Real> &cons 
 {
  ...
  Real xp = radp*cos(phip)*sin(thetap);
  Real yp = radp*sin(phip)*sin(thetap);
  Real zp = radp*cos(thetap);
      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        Real x1 = pmb->pcoord->x1v(i);
          ...
          if (distance <= accretion_rad1) {
            ...
            const Real gas_mom2 = u(IM2,k,j,i);
            const Real gas_mom3 = u(IM3,k,j,i);
            Real gas_vel1 = gas_mom1/gas_rho;
            Real gas_vel2 = gas_mom2/gas_rho;
            Real gas_vel3 = gas_mom3/gas_rho;
	   ...
	   if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
            //cout << "gas_vel2 before adv: "<< gas_vel2  << endl;
             //if (pmb->porb->orbital_advection_defined)
             //  gas_vel2 += vK(pmb->porb, x1, x2, x3);
             gas_vel2 += Omega0/std::sqrt(1.0+gmp)*x1;
             //cout << "gas_vel2 after adv: "<< gas_vel2  << endl;
             //cout << "vK: "<< vK(pmb->porb, x1, x2, x3)  << endl;
	     Real vx = gas_vel1*cos(x2) - gas_vel2*sin(x2);
	     Real vy = gas_vel1*sin(x2) + gas_vel2*cos(x2);
             Real vz = gas_vel3;
             delta_gas_am = delta_gas_dens*radp*gas_vel2;
           } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
             //if (pmb->porb->orbital_advection_defined)
             //  gas_vel3 += vK(pmb->porb, x1, x2, x3);
             gas_vel3 += Omega0/std::sqrt(1.0+gmp)*x1;
	     Real vx = gas_vel1*sin(x2)*cos(x3) + gas_vel2*cos(x2)*cos(x3) - gas_vel3*sin(x3);
	     Real vy = gas_vel1*sin(x2)*sin(x3) + gas_vel2*cos(x2)*sin(x3) + gas_vel3*cos(x3);
             Real vz = gas_vel1*cos(x2) - gas_vel2*sin(x2);
             delta_gas_am = delta_gas_dens*radp*gas_vel3;
           }
...
Real PlanetAccretionEdotHistory(MeshBlock *pmb,  int iout)
 {
  ...
  Real thetap  = PS[np].getTheta();
  Real gmp    = PS[np].getMass();
  Real vrp    = PS[np].getVr();
  Real vpp    = PS[np].getVp();
  Real vtp    = PS[np].getVt();
  ...
  Real yp = radp*sin(phip)*sin(thetap);
  Real zp = radp*cos(thetap);
  Real vxp = vrp*sin(thetap)*cos(phip)+vtp*cos(thetap)*cos(phip)-vpp*sin(phip);
  Real vyp = vrp*sin(thetap)*sin(phip)+vtp*cos(thetap)*sin(phip)+vpp*cos(phip);
  Real vzp = vrp*cos(thetap)-vtp*sin(thetap);
...
//#pragma omp simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
	   ...
	   if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
	     ...
	     Real vy = gas_vel1*sin(x2) + gas_vel2*cos(x2);
             Real vz = gas_vel3;
             delta_gas_ene = delta_gas_dens*(vx*vxp + vy*vyp + vz*vzp);
           } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
             //if (pmb->porb->orbital_advection_defined)
             //  gas_vel3 += vK(pmb->porb, x1, x2, x3);
             gas_vel3 += Omega0/std::sqrt(1.0+gmp)*x1;
