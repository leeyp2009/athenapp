...
bool error_output;
Real tau_relax, rs, gmp, pp_time, t0pot, omega_p;
Real xp, yp;
...
void GravityPlanet(MeshBlock *pmb, const Real time, const Real dt,
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  d0  = pin->GetOrAddReal("problem", "d0",  1.0e-4);  // initial disk density
  xp  = pin->GetOrAddReal("problem", "xp",  5.0e-2);  // initial x-coordinate of planet
  yp  = pin->GetOrAddReal("problem", "yp",  5.0e-2);  // initial y-coordinate of planet
  pp_time  = pin->GetOrAddReal("problem", "pp_time",  30.0);  // planet mass initial growth time
    ...
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        if (ipert == 1) {
          ...
          if (nu_iso>0.0) {
            phydro->u(IM1,k,j,i) += -qshear*nu_iso;
          }
...
void MySource(MeshBlock *pmb, const Real time, const Real dt,
              ...
              AthenaArray<Real> &cons, AthenaArray<Real> &cons_df, AthenaArray<Real> &cons_scalar)
{
  GravityPlanet(pmb, time, dt, prim, bcc, cons);
  return;
}
...
#pragma omp simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        x_dis = pmb->pcoord->x1v(i)-xp;
        y_dis = pmb->pcoord->x2v(j)-yp;
...
void ForceFromPlanet(Real xp, Real yp, Real xc, Real yc, Real mc 
     const Real time, const Real dt)
{
 Real fx, fy;
 Real distance;
 distance = SQR(xp - xc)+SQR(yp - yc)
 fx = (xc - xp)/POW(distance,1.5)*mc
 fy = (yc - yp)/POW(distance,1.5)*mc
 xp = dt*acc_x;
 yp = dt*acc_y;
