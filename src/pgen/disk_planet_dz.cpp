...
Real nu_alpha_active;
Real nu_alpha_dead;
Real r_alpha_split_inner;
Real r_alpha_split_outer;
Real delta_alpha;
Real alphaslope;

Real NuAlphaProfile(Real r) {
    // 内边界过渡函数（从活跃区到死区）
    Real inner_step = 0.5 * (1.0 - std::tanh((r - r_alpha_split_inner) / delta_alpha));

    // 外边界过渡函数（从死区到线性增长区）
    Real outer_step = 0.5 * (1.0 + std::tanh((r - r_alpha_split_outer) / delta_alpha));

    // 计算最终 alpha 值
    // 1. r < r_inner: alpha_active (由 inner_step = 1, outer_step = 0 控制)
    // 2. r_inner < r < r_outer: 平滑降至 alpha_dead
    // 3. r > r_outer: 以 alphaslope 斜率上升 (由 outer_step = 1 使得该项生效)
    Real alpha = nu_alpha_dead + (nu_alpha_active - nu_alpha_dead) * inner_step 
                 + alphaslope * (r - r_alpha_split_outer) * outer_step;
...
inv_rad_planet, t0_planet, t_end_planet, Pp_time, vk_planet, omega_planet, inv_omega_planet, cs_planet, nu_alpha,
gm0, r0, rho0, dslope, p0_over_r0, pslope, gamma_gas, beta, gMth,
...
void DiskInnerX1_2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                  FaceField &b, Real time, Real dt,
                  int il, int iu, int jl, int ju, int kl, int ku, int ngh);
void DiskInnerX1_Diode(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                       FaceField &b, Real time, Real dt,
                       int il, int iu, int jl, int ju, int kl, int ku, int ngh);
...
//   dffloor        = pin->GetOrAddReal("dust",  "dffloor", (1024*(float_min)));
  nu_alpha_active = pin->GetOrAddReal("problem", "nu_alpha_active", 0.01);
  nu_alpha_dead = pin->GetOrAddReal("problem", "nu_alpha_dead", 0.01);
  r_alpha_split_inner = pin->GetOrAddReal("problem", "r_alpha_split_inner", 0.7);
  r_alpha_split_outer = pin->GetOrAddReal("problem", "r_alpha_split_outer", 1.3);
  alphaslope = pin->GetOrAddReal("problem", "alphaslope", 0.01);
  delta_alpha = pin->GetOrAddReal("problem", "delta_alpha", 0.1);
...
if (nPlanet==2){
  ...
  if (mesh_bcs[BoundaryFace::inner_x1] == GetBoundaryFlag("user")) {
    ...
    EnrollUserBoundaryFunction(BoundaryFace::inner_x1, DiskInnerX1_Diode); // 
    } else {
  ...
  //if ((gmp>0.0)&&(!res_flag)) { }
    ...
    for (int n=0;n<nPlanet;n++) {
      ...
      for (int i=0; i<mesh_size.nx1; i++) {
        ...
        Real vis_vel_r     = -1.5*(NuAlphaProfile(x1max)*p0_over_r0)*std::pow(x1max/r0,pslope+0.5);
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        Real vis_vel_r = -1.5*(NuAlphaProfile(rad)*cs_square/omega_dyn/rad);
...
//#pragma omp simd
  ...
  //}
            ...
            Real cs_square = PoverRho(rad, phi, z);
            Real vis_vel_r = -1.5*(NuAlphaProfile(rad)*cs_square/rad/omega_dyn);
...
#pragma omp simd
        for (int i=is; i<=ie; ++i) {
          ...
          if (rad >= x1min && rad < radius_inner_damping) {
            ...
            Real cs_square = PoverRho(rad, phi, z);
            Real vis_vel_r = -1.5*(NuAlphaProfile(rad)*cs_square/omega_dyn(i)/rad);
    ...
    for (int k=kl; k<=ku; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=1; i<=ngh; ++i) {
          ...
          Real nu     = NuAlphaProfile(rad)*cs_square/omega_dyn;
          Real nu_in     = NuAlphaProfile(rad_in)*cs_square_in/omega_dyn_in;
          prim(IDN,k,j,il-i) = prim(IDN,k,j,il)*nu_in/nu;
          //prim(IDN,k,j,il-i) = dfloor;
          vel = VelProfileCyl_gas(rad,phi,z);
          vis_vel_r     = -1.5*(NuAlphaProfile(rad)*cs_square/rad/omega_dyn);
    ...
    for (int k=kl; k<=ku; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=1; i<=ngh; ++i) {
          ...
          Real vis_vel_r   = -1.5*(NuAlphaProfile(rad_ghost)*cs_square/omega_dyn/rad_ghost);
...
void DiskOuterX1(MeshBlock *pmb,Coordinates *pco, AthenaArray<Real> &prim,
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
    for (int k=kl; k<=ku; ++k) {
	  ...
	  Real nu = NuAlphaProfile(rad)*cs_square/omega_dyn;
	  Real nu_rout = NuAlphaProfile(rad_out)*cs_square_rout/omega_dyn_rout;
          Real lmom = vel*rad;
...
//----------------------------------------------------------------------------------------
//! \fn void DiskInnerX1_HydroDiode(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
//!                                 FaceField &b, Real time, Real dt,
//!                                 int il, int iu, int jl, int ju, int kl, int ku, int ngh)
//! \brief A "diode" (outflow-only) boundary condition for hydro variables at Inner X1.
    ...
    for (int k=kl; k<=ku; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=1; i<=ngh; ++i) {
          ...
          // Replace analytical values with copy & diode condition
          gas_rho_ghost  = prim(IDN, k, j, il);
          gas_vel1_ghost = (prim(IM1, k, j, il) > 0.0) ? 0.0 : prim(IM1, k, j, il);
          gas_vel2_ghost = prim(IM2, k, j, il);
          gas_vel3_ghost = prim(IM3, k, j, il);

          if (NON_BAROTROPIC_EOS) {
            Real &gas_pres_ghost = prim(IEN, k, j, il-i);
            gas_pres_ghost       = prim(IEN, k, j, il);
          }
